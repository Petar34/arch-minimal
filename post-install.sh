...
# 6. GRUB instalacija (ako koristiš NVMe disk, prilagodi ako treba)
...
# 12. Autologin startx na tty1
echo '[ -z "$DISPLAY" ] && [ "$(tty)" = "/dev/tty1" ] && exec startx' >> /home/admin/.bash_profile
chown admin:admin /home/admin/.bash_profile

echo -e "\n\033[1;32m[ZAVRŠENO] post-install.sh za Debian + i3 je gotov. Pokreni reboot!\033[0m"#!/bin/bash
set -e
